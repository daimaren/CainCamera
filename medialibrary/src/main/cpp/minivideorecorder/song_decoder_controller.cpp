...
void* LiveSongDecoderController::startDecoderThread(void* ptr) {
	ALOGI("enter LiveSongDecoderController::startDecoderThread");
...
//	ALOGI("enter LiveSongDecoderController::destoryProduceThread ....");
	isRunning = false;
	isSuspendFlag = false;
	void* status;
	int getLockCode = pthread_mutex_lock(&mLock);
...
void LiveSongDecoderController::destroy() {
	ALOGI("enter LiveSongDecoderController::destroy");
	this->destroyDecoderThread();
	ALOGI("after destroyDecoderThread");
