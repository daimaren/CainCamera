...
int Resampler::init(int _src_rate, int _dst_rate, int _max_src_nb_samples, int src_channels, int to_channels) {
	...
	ALOGI("src_channels is %d to_channels is %d", src_channels, to_channels);
...
//	ALOGI("av_get_channel_layout_nb_channels...\n");
...
//		ALOGI("Could not allocate destination samples\n");
		destroy();
	}

//	ALOGI("av_samples_alloc_array_and_samples return:%d\n", ret);
	/* compute destination number of samples */
	dst_nb_samples = av_rescale_rnd(swr_get_delay(swr_ctx, src_rate) + src_nb_samples, dst_rate, src_rate, AV_ROUND_UP);
//	ALOGI("Init dst_nb_samples as %d\n",dst_nb_samples);
