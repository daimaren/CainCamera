...
void CircleFrameTextureQueue::init(int width, int height, int queueSizeParam){
	ALOGI("enter CircleFrameTextureQueue::init width is %d height is %d queueSize is %d", width, height, queueSizeParam);
	if(queueSizeParam < 5){
		ALOGI("circle video packet queue min size is 5");
		return;
	}
...
//	ALOGI("leave CircleFrameTextureQueue::init");
...
//	ALOGI("buildFrameTexture:width is %d height is %d texId is %d", width, height, frameTexture->texId);
	return frameTexture;
}
...
//	ALOGI("enter CircleFrameTextureQueue::lockPushCursorFrameTexture");
	if (mAbortRequest) {
		return NULL;
	}
	int getLockCode = pthread_mutex_lock(&mLock);
...
//	ALOGI("enter CircleFrameTextureQueue::unLockPushCursorFrameTexture");
	if (mAbortRequest) {
		return;
	}
...
//	ALOGI("enter CircleFrameTextureQueue::clear");
	pthread_mutex_lock(&mLock);
	isAvailable = false;
	pullCursor = head;
	pushCursor = head;
	pthread_mutex_unlock(&mLock);
//	ALOGI("leave CircleFrameTextureQueue::clear");
}
...
bool CircleFrameTextureQueue::checkGlError(const char* op) {
	...
	for (error = glGetError(); error; error = glGetError()) {
		ALOGI("error::after %s() glError (0x%x)\n", op, error);
		return true;
	}
	return false;
}
...
//	ALOGI("%s ~CircleFrameTextureQueue ....", queueName);
	flush();
...
//	ALOGI("\n %s flush .... and this time the queue size is %d \n", queueName, queueSize);
	pthread_mutex_lock(&mLock);
//	ALOGI("success get a lock in CircleFrameTextureQueue flush method...");
