...
RecordingPreviewController::RecordingPreviewController() {
    ...
    isEncoding = false;
    queue = new MessageQueue("RecordingPreviewController message queue");
...
void RecordingPreviewController::updateTexMatrix(float *texMatrix) {
    //TODO:
//	memcpy(mTexMatrix, texMatrix, sizeof(float)*16);
}

int RecordingPreviewController::setBeautifyParam(int key, float value) {
	if (renderer) {
//		renderer->setBeautifyParam(key, value);
	}
}

void RecordingPreviewController::switchPreviewFilter(int filterType, byte *acvBuffer,
                                                       int bufferSize) {
...
void RecordingPreviewController::startEncoding(int width, int height, int videoBitRate,
...
void RecordingPreviewController::resetRenderSize(int screenWidth, int screenHeight) {
    LOGI("RecordingPreviewController::resetSize screenWidth:%d; screenHeight:%d", screenWidth,
         screenHeight);
    this->screenWidth = screenWidth;
    this->screenHeight = screenHeight;
}

void RecordingPreviewController::switchPauseRecordingPreviewState() {
    pauseFlag = true;
}

void RecordingPreviewController::switchCommonPreviewState() {
    continueFlag = true;
}
...
void RecordingPreviewController::switchFilter() {
